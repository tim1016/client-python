...
class ThreeMinutePatternValidator:
    ...
    def validate_pattern_changes(self, df, offset=0):
        ...
        if not validation_passed:
            print(f"\n! Warning: Discrepancy of ${discrepancy:.6f} detected!")
    ...
    def compare_with_analysis_results(self, symbol, offset=0):
        ...
        if offset in self.validation_results:
            ...
            print(f"\nConsistency Check:")
            print(f"Pattern count matches: {'[YES]' if count_match else '[NO]'}")
            print(f"Total change matches: {'[YES]' if total_match else '[NO]'}")
    ...
    def create_validation_visualization(self, symbol):
        ...
        print(f"\n[OK] Validation chart saved to: {output_file}")
    ...
    def export_validation_report(self, symbol):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write("-"*80 + "\n")
            f.write(f"{'Offset':<10} {'Patterns':<10} {'Pattern Sum':<15} {'Actual Sum':<15} {'Discrepancy':<15} {'Status':<10}\n")
            f.write("-"*80 + "\n")
            ...
            f.write("\n")
            f.write("OVERALL VALIDATION: " + ('[ALL OFFSETS PASSED]' if all_passed else '[SOME OFFSETS FAILED]') + "\n")
    ...
    def run_complete_validation(self, symbol):
        ...
        print("\n[Generating] Creating validation visualization...")
        self.create_validation_visualization(symbol)
        
        # Export report
        print("\n[Generating] Exporting validation report...")
        ...
        if all_passed:
            print("[SUCCESS] All offsets passed validation!")
            ...
            print(f"[WARNING] Validation failed for offsets: {failed_offsets}")
